...
describe("T064: Terraform Module & Configuration Tests", () => {
  ...
  describe("モジュール内容検証 - Compute", () => {
    ...
    it("Lambda 関数または API Gateway が定義されている", () => {
      if (computeMainContent) {
        const hasLambdaOrAPI = computeMainContent.match(
          /resource\s+"aws_lambda_function"|resource\s+"aws_apigatewayv2_api"|resource\s+"aws_api_gateway_rest_api"/
        );
  ...
  describe("モジュール内容検証 - Data", () => {
    ...
    it("グローバルセカンダリインデックスが考慮されている", () => {
      ...
      const hasGSI =
        dataMainContent.includes("global_secondary_index") ||
    ...
    it("Terraform init が成功する", () => {
      ...
      try {
        const result = execSync(
          `cd ${terraformDir} && terraform init -backend=false -upgrade 2>&1`,
          {
            encoding: "utf8",
  ...
  describe("ドキュメント検証", () => {
    ...
    it("各モジュールの README.md が Terraform ドキュメント形式に従う", () => {
      ...
      console.log(
        `📚 ドキュメント化されたモジュール: ${documentedModules}/${requiredModules.length}`
      );
    });
  });
