...
describe('AWS Integration Tests - DynamoDB', () => {
  ...
  describe('DynamoDB クライアント - バッチ操作', () => {
    ...
    it('複数項目を一括取得 - batchGetItems', async () => {
      const todos = [
        createTestTodo('Get batch 1'),
        createTestTodo('Get batch 2'),
      ];
    ...
    it('複数項目を一括削除 - batchDeleteItems', async () => {
      const todos = [
        createTestTodo('Delete batch 1'),
        createTestTodo('Delete batch 2'),
      ];
...
describe('AWS Integration Tests - DynamoDB Repository', () => {
  ...
  describe('IAsyncTodoRepository インターフェース実装', () => {
    ...
    it('すべての Todo を取得 - findAll', async () => {
      const todos = [
        createTestTodo('Find all 1'),
        createTestTodo('Find all 2'),
      ];
    ...
    it('複数 Todo を一括削除 - deleteMany', async () => {
      const todos = [
        createTestTodo('Delete many 1'),
        createTestTodo('Delete many 2'),
      ];
  ...
  describe('Lambda クライアント - 基本動作', () => {
    ...
    it('Lambda 関数の健全性チェック', async () => {
      try {
        ...
        console.warn('⚠️  Lambda healthCheck failed (expected if LocalStack not running):', error.message);
      }
    }, 10000);
  });
