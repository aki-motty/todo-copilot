    ...
    <div data-testid="subtask-list">
      {subtasks.map((s: any) => (
        <div key={s.id} data-testid={`subtask-${s.id}`}>
          {s.title}
          <button type="button" onClick={() => onToggleSubtask(s.id)}>
            Toggle Subtask
          </button>
...
describe("TodoItem Component", () => {
  ...
  it("renders tags", () => {
    ...
    const summaryTag = screen
      .getAllByText("Summary")
      .find((el) => el.classList.contains("todo-tag"));
    expect(summaryTag).toBeInTheDocument();

    const researchTag = screen
  ...
  it("shows add subtask form when expanded", () => {
    render(<TodoItem todo={mockTodo} onAddSubtask={mockOnAddSubtask} />);
