...
test.describe('Multiple Todo Operations', () => {
  ...
  test('should handle toggling multiple todos', async ({ page }) => {
    ...
    await expect(page.locator('.todo-item', { hasText: todoTitles[1] }).locator('input[type="checkbox"]')).not.toBeChecked();
  ...
  test('should handle mixed operations', async ({ page }) => {
    ...
    // Delete first todo
    await page.locator('.todo-item', { hasText: `Mixed Op 1 ${timestamp}` })
      .locator('button[aria-label="Delete todo"]').click();
    await page.waitForTimeout(500);
