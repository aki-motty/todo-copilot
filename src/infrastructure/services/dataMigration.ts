...
/**
 * Interface for localStorage todo format (from Phase 1)
 * May differ from TodoResponseDTO
 */
 ...
 */
export function getLocalStorageTodos(): LocalStorageTodo[] {
  try {
    const stored = localStorage.getItem("todos");
    if (!stored) {
      return [];
    }
    return JSON.parse(stored) as LocalStorageTodo[];
  } catch (error) {
 ...
 */
export function hasLocalStorageTodos(): boolean {
 ...
 */
export function clearLocalStorageTodos(): void {
  try {
