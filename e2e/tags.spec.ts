...
test.describe('Task Tags', () => {
  ...
  test('should add and remove tags', async ({ page }) => {
    ...
    await expect(todoItem.locator('.todo-tag').nth(1)).toContainText('Research', { timeout: 30000 });

    // Remove tag - wait for network to settle
    const removePromise = page.waitForResponse(resp => resp.url().includes('/tags/') && resp.request().method() === 'DELETE', { timeout: 30000 });
  ...
  test('should persist tags after reload', async ({ page }) => {
    ...
    await expect(reloadedTodoItem.locator('.todo-tag')).toContainText('Summary', { timeout: 30000 });
  });
});
