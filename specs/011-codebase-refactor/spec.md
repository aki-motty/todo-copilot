...
1. **Given** 現在のスクリプト名, **When** 命名規則を確認, **Then** `dev:*`、`build:*`、`test:*`、`db:*` などの一貫したプレフィックスが使われている
...
- **FR-001**: テストカバレッジがステートメント、ブランチ、行、関数すべてで80%以上を達成すること
...
- **SC-001**: テストカバレッジが全カテゴリで80%以上（現在: 32%）
