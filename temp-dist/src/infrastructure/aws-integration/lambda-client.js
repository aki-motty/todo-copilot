...
export class LambdaClientService {
    ...
    async invokeSync(functionName, payload) {
        try {
            ...
            if (response.FunctionError) {
                throw new Error(`Lambda function error: ${response.FunctionError}. LogResult: ${response.LogResult}`);
            }
            if (!response.Payload) {
                return {};
            }
            // Payload を文字列から解析
            const payloadStr = response.Payload instanceof Uint8Array
     ...
     * @returns 検証結果
     */
    async invokeDryRun(functionName, payload) {
        try {
            const command = new InvokeCommand({
                FunctionName: functionName,
 ...
 */
export function resetLambdaClient() {
    instance = null;
}
