    ...
    async request(method, path, body, options) {
        ...
        try {
            ...
            if (error instanceof TypeError && error.message.includes("Failed to fetch")) {
                const networkError = new NetworkError(`Network error: Unable to connect to ${url}. Check if the server is running.`);
                this.logger.error(`${method} ${path} - Network Error`, networkError);
                throw networkError;
            }
            if (error instanceof Error && error.name === "AbortError") {
                const timeoutError = new TimeoutError(`Request timeout: ${method} ${path} took longer than ${timeout}ms`);
