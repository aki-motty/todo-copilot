...
export class AsyncApiTodoRepository {
    constructor(baseUrl) {
        this.logger = createLogger("AsyncApiTodoRepository");
    ...
    async findById(id) {
        try {
            ...
            const todoDto = await this.httpClient.get(`/todos/${id}`);
    ...
    async findAll() {
        try {
            ...
            this.logger.error("Error finding all todos", error instanceof Error ? error : new Error(String(error)));
            throw this.mapApiError(error);
        }
    }
    /**
    ...
    async save(todo) {
        try {
            ...
            this.logger.error("Error saving todo", error instanceof Error ? error : new Error(String(error)));
            throw this.mapApiError(error);
        }
    }
    /**
    ...
    async count() {
        try {
            ...
            const response = await this.httpClient.get("/todos/count");
