...
export class Todo {
    ...
    static fromPersistence(id, title, completed, createdAt, updatedAt, subtasks = []) {
        ...
        const todoTitle = TodoTitle.create(title);
        return new Todo(todoId, todoTitle, completed, new Date(createdAt), new Date(updatedAt), subtasks);
    }
    /**
    ...
    toggleCompletion() {
        ...
        return new Todo(this._id, this._title, newCompleted, this._createdAt, new Date(), // Update timestamp
        newSubtasks);
    }
    /**
    ...
    addSubtask(title) {
        ...
        return new Todo(this._id, this._title, this._completed, this._createdAt, new Date(), [...this._subtasks, subtask]);
    }
    /**
     * Remove a subtask from the todo
     */
    removeSubtask(subtaskId) {
        return new Todo(this._id, this._title, this._completed, this._createdAt, new Date(), this._subtasks.filter((s) => s.id !== subtaskId));
    }
    /**
     * Toggle a subtask's completion status
     */
    toggleSubtask(subtaskId) {
        return new Todo(this._id, this._title, this._completed, this._createdAt, new Date(), this._subtasks.map((s) => (s.id === subtaskId ? s.toggleCompletion() : s)));
    }
